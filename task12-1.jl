...
distance(f::Ferry) = round(Int64, abs(f.x) + abs(f.y))
...
function rotate!(f::Ferry, angle)
	f.angle += angle
	f.angle = mod(f.angle, 360)
end

# ╔═╡ 93ee6210-3c91-11eb-3841-a1cd9cf52e7b
function forward!(f::Ferry, Δ)
	f.x += Δ * cos(f.angle/360*2π)
	f.y += Δ * sin(f.angle/360*2π)
end
...
begin
	task1_ferry = Ferry()
	do!(task1_ferry, task1_comms)
	distance(task1_ferry)
end
