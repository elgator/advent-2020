...
function step!(memory, last::Int64, i::Int64)::Int64
	next = nothing
	if !(last ∈ keys(memory))
		next = 0
	else
...
function calc(init::Array{Int64}, limit::Int64=2020)
	memory = Dict()
	next  = 0
...
# ╔═╡ 41f1b0d0-3ec5-11eb-1ca4-ad89595990e8
begin
	@assert calc([0,3,6], 30000000) == 175594
	@assert calc([1,3,2], 30000000) == 2578
	@assert calc([2,1,3], 30000000) == 3544142
	@assert calc([1,2,3], 30000000) == 261214
	@assert calc([2,3,1], 30000000) == 6895259
	@assert calc([3,2,1], 30000000) == 18
	@assert calc([3,1,2], 30000000) == 362
end

# ╔═╡ 4854dd20-3ec6-11eb-3f20-c980a3e24d55
part2 = calc([8,0,17,4,1,12], 30000000)
